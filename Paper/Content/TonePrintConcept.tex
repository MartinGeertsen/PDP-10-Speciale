...
In collaboration with multiple famous guitarists and bas players did TC Electronics create effects for a number of pedals used by the artists them selves \parencite{PDF:TonePrintAnalyse} called TonePrints. The TonePrints are created by tweaking all the parameters which are able for the given pedal, which isn't just the ones that are represented by the physical knobs. When the creators of the TonePrint is satisfied with the result is the TonePrint uploaded to the TonePrint library, enabling every one with a pedal of the same type the TonePrint was designed fore, to download the TonePrint and use the effect to sound just like the artist whom created it.
